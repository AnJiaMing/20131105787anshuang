...
int main(int argc, char** argv){
    ...
    Ptr<BackgroundSubtractorGMG> fgbg = Algorithm::create<BackgroundSubtractorGMG>("BackgroundSubtractor.GMG");//首先定义一个BackgroundSubtractorGMG结构，并设置它最开始初始化帧数为20，算法阀值参数为0.7
    if (fgbg.empty()){
    ...
    VideoCapture cap;//打开视频文件到cap中
    cap.open("d:\\录屏_20150624_185346.avi");
    if (!cap.isOpened()){
        std::cerr << "Cannot read video. Try moving video file to sample directory." << std::endl;
        return -1;
    }
    namedWindow("FG Segmentation", WINDOW_NORMAL);	//取出图像中数据，使用(*fgbg)一帧一帧的进行跟踪处理，将处理后跟踪到的运动物体位置，保存到fgmask中。接着使用cvSegmentFGMask
    ...
    return 0;//对高速公路上的车流量进行计数，并创建对话框显示车流数
}
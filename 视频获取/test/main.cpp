...
int main( int argc, char** argv)
{
	...
	capture = cvCaptureFromFile("d://野生动物.avi");
	if (capture){
		for(;;){ 
	...
	//CvCapture *avi=cvCreateFileCapture("abc.avi");   对视频桢的提取
 ...
 void capture_frame(){
     ...
     CvCapture* capture = cvCaptureFromAVI("D:\\野生动物.avi");
    IplImage* img = 0; 
    int i; 
     i = 0;
     ...
     while(cvGrabFrame(capture)){
         ...
         sprintf(FileName,"D:\\程序设计\\程序\\程序2-opencv配置程序\\视频桢提取\\视频桢提取文件\\%04d.jpg",i);
...
{    
	...
	int thresh = 80;  
//载入图像  
	img0=cvLoadImage("d:\\0002.jpg",0);//不可以再一次用IplImage定义，因为其已经是全局变量  
